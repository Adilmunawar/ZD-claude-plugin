...
| `npm error 404 @adilmunawar/zd-tools is not in this registry` | The scoped name resolves to npmjs, where it is not published | `npm i -g github:Adilmunawar/ZD-claude-plugin` — no registry, no token. Or the release tarball URL, or configure `~/.npmrc` for GitHub Packages |
